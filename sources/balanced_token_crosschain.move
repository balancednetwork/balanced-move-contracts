module balanced::balanced_token_crosschain {
    ...
    use balanced_token::balanced_token::{Self, BALANCED_TOKEN};
    ...
    entry fun configure(_: &AdminCap, treasury_cap: TreasuryCap<BALANCED_TOKEN>, xcall_manager_config: &XcallManagerConfig, storage: &XCallState, witness_carrier: WitnessCarrier, icon_baln: String, version: u64, ctx: &mut TxContext ){
    ...
    entry fun cross_transfer(
        ...
        balanced_token::burn(get_treasury_cap_mut(config), token);
        let from = ctx.sender();
        ...
        xcall::send_call(xcall_state, fee, get_idcap(config), config.icon_baln, envelope::encode(&envelope), ctx);
    }
    ...
    entry fun execute_call(config: &mut Config, xcall_manager_config: &XcallManagerConfig, xcall:&mut XCallState, fee: Coin<SUI>, request_id:u128, data:vector<u8>, ctx:&mut TxContext){
        ...
        if (verified && method == CROSS_TRANSFER && from == network_address::from_string(config.icon_baln)){
            ...
            balanced_token::mint(get_treasury_cap_mut(config), address_from_hex_string(&to),  amount, ctx);
    ...
    entry fun execute_rollback(config: &mut Config, xcall:&mut XCallState, sn: u128, ctx:&mut TxContext){
        ...
        balanced_token::mint(get_treasury_cap_mut(config), to, amount,  ctx);
        xcall::execute_rollback_result(xcall,ticket,true)
    }
    
    fun get_treasury_cap_mut(config: &mut Config): &mut TreasuryCap<BALANCED_TOKEN>{
