module balanced::sicx_crosschain {
    ...
    public struct Config has key, store{
        ...
        xcall_manager_id: ID, 
        xcall_id: ID,
        balanced_treasury_cap: TreasuryCap<SICX>
    }
    ...
    entry fun execute_call(config: &mut Config, xcall_manager_config: &XcallManagerConfig, xcall:&mut XCallState, fee: Coin<SUI>, request_id:u128, data:vector<u8>, ctx:&mut TxContext){
        ...
        if (verified && method == CROSS_TRANSFER && from == network_address::from_string(config.icon_sicx)){
            ...
            sicx::mint(get_treasury_cap_mut(config), address_from_hex_string(&to),  amount, ctx);
    ...
    fun translate_incoming_amount(amount: u128): u64 {
     ...
     #[test_only]
    public fun get_treasury_cap_for_testing(config: &mut Config): &mut TreasuryCap<SICX> {
        &mut config.balanced_treasury_cap
    }
