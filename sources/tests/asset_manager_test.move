...
module balanced::asset_manager_test {
    ...
    fun withdraw_to_execute_call() {
        ...
        let deposited = coin::mint_for_testing<BALANCED_DOLLAR>(bnusd_amount*2, scenario.ctx());
        deposit(&mut xcall_state, &mut config, &xcallManagerConfig, deposit_fee, deposited, bnusd_amount*2, option::none(), option::none(), scenario.ctx());
    ...
    fun rollback_execute_call() {
        ...
        deposit( &mut xcall_state, &mut config, &xcallManagerConfig, deposit_fee1, deposited1, bnusd_amount*2, option::none(), option::none(), scenario.ctx());
        xcall::handle_message(&mut xcall_state, &conn_cap, from_nid, message, scenario.ctx());
